...
export default class Navigator extends Component {
  ...
  navigator = {
    ...
    push: async (screenId, props, transitionConfig) => {
      ...
      return this.navigatorAction(async () => {
        ...
        if (transitionConfig.wait) {
          await sleep(transitionConfig.wait);
        }
  ...
  renderScreen = (stackItem, index) => {
    ...
    const st = index < stack.length - 2 && needShowId !== stackItem.uuid ? styleHiddenScreen : styleAbsoluteFillObject;
